...
- **Mono 全般**: 本移植は Mono 非対象(36 §4 G1)で Mono 上の検証は行わない。
  Mono との接点を持ち得る変更は 2 系統ある: N2.3(ncc 本体。歴史的に Mono でも
  ビルドされてきたソース)と、N2.2 / N2.4(engine 共有ソース。SharpDevelop binding
  経由で VS 以外の IDE 環境 — SharpDevelop / 派生の Mono ベース環境 — から消費されて
  いた可能性がある)。N2.2 / N2.4 は public API シグネチャと既存挙動の互換
  (既存で解決できていた結果を変えない)に注意する。N2.3 は既定挙動を変えない
  opt-in のため、ソース互換(使用する言語機能・API)以外の新規リスクは想定しない。
