...
## 10. 追記(コミット後のフォローアップ): `dotnet-port\rsp\` の untrack 化

本 WP のコミット(ddc3c2487)後、手動テスト用の再パッケージ(WP-N1 §10 の封緘手順)を
開始した時点で、`build-stage2-core.ps1` が tracked の `rsp\stage2\*.rsp` を再生成して
ワークツリーを汚すことを PO が指摘。批判的検討の結果、**`dotnet-port\rsp\` をフォルダごと
git 管理から外した**(`.gitignore` 追加 + `git rm --cached`。ファイルはディスクに残る)。

根拠:

1. rsp はスクリプトが**実行のたびに全文再生成する導出物**(ソース一覧・参照・出力先)で、
   一次情報はスクリプト側にある。内容は機械固有の絶対パスと shared framework の
   パッチ版(例 `10.0.9`)を含み、リポジトリで確定させる対象として不適格。
2. tracked のままだと「確定コミット → その HEAD で Stage 再構築 → pack → 封緘
   (clean tree 必須)」という WP-N1 §10 の手順と**構造的に矛盾**する(Stage 再構築が
   必ず tree を汚す。37 §9-6 の「HEAD 状態のまま維持」運用は偶然依存だった)。
3. `rsp\stage3\` は初期のセルフホスト達成コミット(29981a2dc)の名残で、現在は
   どのスクリプトからも参照されない死んだ tracked ファイルだった(stage3 ビルドも
   `-OutDir` 指定で rsp\stage2 側に書く)。stage2 だけでなくフォルダ単位で ignore する
   のはこのため。
4. WP-N6(CI)も Windows 絶対パス入りの checked-in rsp は使えず、スクリプトで再生成する
   前提のため、tracked である必要がない。
...
PO の WSL 実機手動テスト(1.2.623-preview.1 セット + 宣言 hover 修正入り VSIX)で確認:

- R-H 4件・宣言位置 hover(`elem` / `Min`)の表示修正を実機確認。
- provenance 版不一致警告(WP-M6)が「旧 601 SDK × 新 623 server」の組で正しく発報し、
  SDK 更新で解消することを実機確認(機能の実地検証を兼ねた)。
- server-only 修正の検証には「VSIX のみ再生成」で足りる(同梱 Nemerle.dll が SDK と同版なら
  警告なし)ことを確認 — ただし混成セットは pack-release が封緘拒否する(意図どおり)ため、
  クローズ時に全体を再封緘する。
- `array.2[bool]`(多次元配列の型プリンター表記)は仕様どおりと確認(ソース綴り
  `array[2, T]` と異なるのは上流仕様。ソース綴りへの寄せは E9 バックログ候補)。
