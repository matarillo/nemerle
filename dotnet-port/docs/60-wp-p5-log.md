...
- **挿入点の手前が空白だけなら edit を列 0 に置く**(通常のケース)。元の行 = その空白と閉じ波括弧が
  挿入テキストの後ろにそのまま続くので、波括弧の位置は動かない(当初は波括弧の直前に挿入して
  いたが、行の空白とメンバーのインデントが積み上がる欠陥があった — 下記 追記)。
- **挿入点の手前にコードがあるとき**(`class Foo { }` のような 1 行宣言)だけ改行から始め、
  その行の**空白のみ**を末尾に戻す。
...
- ~~**生成されるプロパティ本体のインデントに engine のタブが混じる**~~ → **修正済み(下記 追記)**。
...
## 追記(2026-07-26): インデントの欠陥 2 件を修正(実機確認で発覚)

PO の実機確認で「アクションを実行するとコードは挿入されるが**インデントが適切でない**」と
報告された。原因は 2 件あり、**どちらも「テキスト断片としては正しく、ファイルに入れると崩れる」**
性質だったため、生成テキストだけを見る当初の assertion では捕まっていなかった。

### 1. engine のタブが混ざる(当初「軽微」として残していた判断を撤回)

`SourceGenerator` は生成メンバーの内部ネストに **タブを 1 段 1 個**出す(プロパティの `get`、
メソッド本体)。`Reindent` は行頭に目標インデントを足すだけだったので、結果は
`"    " + "	"` = **タブとスペースの混在**になり、深さが読み手のタブ幅に依存した。

当初は「engine の書式を勝手に変えない」として残したが、**スペース字下げのファイルにタブを
混ぜることは「engine の書式」ではない**ので撤回する。修正は「**engine のネスト段数は保ち、
単位だけ挿入先に合わせる**」= 先頭のタブ 1 個を 1 段分(スペース字下げなら 2 スペース、
タブ字下げならタブ)に置き換える。

### 2. 1 行目だけ 2 桁深くなる(挿入位置の設計ミス)

閉じ波括弧が自分の行にある通常のケースで、edit を**波括弧の直前**(= その行の空白の後ろ)に
置いていたため、**行の空白 + メンバーのインデント**が積み上がり、**最初のメンバーだけ 6 桁**、
残りが 4 桁になっていた。

修正: 挿入点の手前が空白だけなら **edit を列 0 に置く**。元の行(その空白と閉じ波括弧)は
挿入テキストの後ろにそのまま続くので、末尾での空白の再出力も不要になった
(再出力していたぶんが二重になっていた)。行中への挿入(`class Foo { }` の 1 行宣言)だけは
従来どおりキャレット位置に改行から挿入し、その行の**空白のみ**を戻す
(コードまで戻していたのは誤りで、**単体テストがその誤りを期待値として固定していた** — 修正した)。

### 修正後の実測(`samples`/playground の `P5_CodeAction.n` に適用)
...
```
    ...
    public Draw(at : WpP.GridPoint) : void
    {
      _ = at;
...
既存メンバーと同じ 4 桁、本体が 6 桁、クラスの閉じ波括弧は 2 桁のまま、タブ混入なし。

### テストの補強

- unit: タブ→ステップ変換(スペース字下げ/タブ字下げの双方)、2 段ネスト、**タブが 1 つも
  残らないこと**、列 0 への配置、行中挿入がコードを重複させないこと、
  **「適用後の 1 行」の形**を固定。
- raw LSP シナリオ: 生成テキストに**タブが無いこと**と、**生成された 2 つのメンバー宣言が
  どちらもクラス本体のインデント(2 桁)にあること**を追加。断片ではなくファイルの形を見る。

**教訓**: 生成コードの assertion は「必要な文字列を含む」では足りない。**適用後の行の形**
(列位置・字下げ文字の種類)を見ないと、コンパイルは通るが人間には読めない出力を通してしまう。
WP-P5 の受け入れ基準「適用後にエラーが 0」も、この 2 件は素通りさせていた。
