...
## 追記2(2026-07-26): 「ハイライトが一瞬で消える」= 既知の挙動として記録 + ログ量の是正

`wordPattern` の修正(追記1)で documentHighlight は要求・応答されるようになったが、実機(WSL +
VS Code)で **まれに「シングルクリックで一瞬光ってすぐ消える」**という症状が残った。原因の完全な
特定には至っていないため、**現時点の事実と有力な仮説を記録して優先度を下げる**(PO 合意)。
ただし調査中に判明した**我々自身のログ量の問題は独立に修正した**(下記)。

### 事実(実測)

サーバー側は繰り返し除外できている:

- 返す 7 件の range はすべて単一行・識別子ぴったり・kind も正しい(write 1 / read 6)。
- 起動直後の連射 60/60、`semanticTokens/full` と同時実行 25/25 で欠けなし。
- capability の再登録・解除は起動時の 8 件のみ(20 秒監視)。
- **症状発生時、クリック後にサーバーへの再要求は来ない** = VS Code が自分で decoration を捨てている。
- **拡張機能なし + クリーンプロファイルの実 VS Code(統合テストハーネス)では再現しない**
  (F7 = `editor.action.wordHighlight.next` が直後も 3 秒後も動作)。

PO 報告による切り分け:

- **他言語(JavaScript)では安定**している。
- キーボードでキャレットを入れても同じ(マウス固有ではない)。
- 消えた後は **F7 も効かない** = VS Code は描画だけでなく内部状態ごと破棄している。
- **Output パネルを閉じると正常化し、その後 Output を再表示しても正常なまま。**
- 拡張の有効化/無効化を繰り返すうちに発生頻度が下がった。

### 有力な仮説

**Output パネル自身がエディターであり、そこへの追記が code editor 側の word highlighter の
状態を落としている。** これだと観測が全部説明できる:

| 観測 | 説明 |
|---|---|
| Output 表示中に起きる | ログ行の追記で Output エディターが更新される |
| 起動直後に多い | サーバーが最も饒舌な時間帯(リビルド・refresh・診断) |
| 閉じると直る / 再表示しても直ったまま | そのときにはサーバーが静かになっている |
| **JavaScript は安定** | TS サーバーは**要求ごとに Output へ書かない**。我々は書いていた |

未確定であることを明示しておく: ハーネスで Output パネルを開いた状態の再現は**試していない**。
確定させたい場合の次の一手はそれ。

### 独立に実施した修正: 要求ごとのログを trace 限定にした

症状の主犯候補である以前に、**要求ごとに info 行を出すのは Output を読めなくする**ので単体で
問題だった(WP-O5a がログを足した目的 = ユーザーが自分で切り分けられること、を損なう)。

- `MessageType.Log` は LSP の最下位だが **vscode-languageclient は `[info]` として出す**ので、
  「静かにする」は「送らない」以外に無い。→ `ServerLog.Trace()` を追加。
- **クライアントの trace 設定で開閉する**: `initialize` の `trace` と `$/setTrace` の両方を見る
  (`nemerle.server.trace` を切り替えると**再起動なしで**即座に効く。実測済み)。
- **trace に落としたもの**(キャレット移動ごとに出る = poll 駆動): hover / signatureHelp /
  documentHighlight / codeAction の「計算できた」「空だった」「対象外だった」「stale/cancelled」。
- **info(既定表示)に残したもの**: rename と formatting(ユーザー操作起点で低頻度)、
  プロジェクトクエリ・engine リビルド・provenance、そして **warning / error 全部**。
  = 無言の失敗は trace を入れなくても見える。

実測: 既定(trace off)で hover + documentHighlight を叩いて **ログ 0 行**、機能は正常
(7 件返る)。`$/setTrace verbose` で即座に復活、`off` で即座に停止。

統合テストは `initialize` に `trace: "verbose"` を送るようにした(= 機能を診断する
ユーザーと同じ状態)。既存のログ assertion はそのまま通る。

### 申し送り

- **描画寿命の追跡は保留**。再開するなら「ハーネスで Output パネルを開いて F7 が死ぬか」から。
- ログ降格により、**症状が消えたかどうかの再評価が必要**(仮説が正しければ頻度はさらに下がる)。
  ただし因果は未確定なので、消えても「直った」と書かないこと。
