...
- 2026-07-23: **WP-O4(配布の器の判断)完了**。nuget.org = **no-go** / VS Code Marketplace =
  **no-go** で確定(GitHub Release を配布器として維持)。両器の一方通行コミット(nuget.org は
  package ID/版の恒久性、Marketplace は publisher identity)を避け、位置づけ(保存中心・少し試せる
  間口・本格採用は非狙い)に整合。公開しないため恒久命名の確定は不要で、runtime package は単一
