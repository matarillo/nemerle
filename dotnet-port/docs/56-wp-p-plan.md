...
WP-P の成果(サーバー + 拡張 `wordPattern` 修正)は**まだ配布していない**。VSIX / server の版は
**0.10.0(未公開)**のまま。配るとすれば `release/1.2.635-preview.3` として一式を発行することに
なる(リリースが版の単位という設計は `pack-release.ps1` 冒頭の note を参照)。
**上記の残課題を抱えたまま発行してよいかの判断は、本節の時点では下していない。**
